...
public interface AbsenceRepository extends JpaRepository<Absence, Long> {
	...
	@Query(value = "SELECT COUNT(*) FROM `absence` a WHERE  a.utilisateur_id=?1 and a.justification=0",nativeQuery = true)
	Long AbsenceNonJustifier(Long utilisateur_id);
	
	@Query(value = "SELECT COUNT(*) FROM `absence` a WHERE  a.utilisateur_id=?1 and a.justification=1",nativeQuery = true)
